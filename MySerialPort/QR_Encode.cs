...
class RS_BLOCKINFO
{
    ushort ncRSBlock;      //纠错的块数
    ushort ncAllCodeWord;  //码字总数
    ushort ncDataCodeWord;	//指定纠错等级下的数据码字数
};

class QR_VERSIONINFO
{
    ushort nVersionNo;         //ver 1~40
    ushort ncAllCodeWord;      //码字总数=数据码字+纠错码字

    ushort[] ncDataCodeWord = new ushort[4];  //指定纠错等级下的数据码字(0=L,1=M,2=Q,3=H)
    ushort ncAlignPoint;       //P61 表E1 校正图形 个数
    ushort[] nAlignPoint = new ushort[6];     //P61 表E1 校正图形	行列坐标
                            //(0=L,1=M,2=Q,3=H)
    RS_BLOCKINFO[] RS_BlockInfo1 = new RS_BLOCKINFO[4];  //纠错块1
    RS_BLOCKINFO[] RS_BlockInfo2 = new RS_BLOCKINFO[4];	//纠错块2
};
...
namespace QR_Encode
{
    ...
    {
        private int QR_MARGIN = 4;
        private int QR_VER1_SIZE = 21;

        private int MAX_ALLCODEWORD = 3706;//400// //P14,P35 数据容量[码字]* (E) (VER:40), 所有码字为8位
        private int MAX_DATACODEWORD = 2956;//400// //P27     最大信息码子(Ver：40-L)，所有码字为8位
        private int MAX_CODEBLOCK = 153;//最大纠错码字 Ver：36.37.38_L_第二块
        private int MAX_MODULESIZE = 177;	// 21:Version=1,最大字符=17(8.5个汉字)

        public int m_nSymbleSize;
        public byte[][] m_byModuleData = new byte[117][];
